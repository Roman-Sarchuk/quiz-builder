    ...
    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
      ...
      <div className="space-y-2">
        {(question.options ?? []).map((option, optionIndex) => {
          const checked = Array.isArray(question.answers) ? question.answers.includes(option) : false;

          return (
            <label key={`${question.id}-${optionIndex}`} className="flex items-center gap-3 text-sm text-zinc-700">
